...
public class Archer extends GameCharacter {
    ...
    //
    // Serializable НЕ указан явно в объявлении Archer, потому что он НАСЛЕДУЕТСЯ
    // от GameCharacter (который реализует Serializable). В Java интерфейсы наследуются:
    // если родительский класс реализует интерфейс, все наследники тоже его реализуют.
    //
    // serialVersionUID — уникальный идентификатор версии класса для механизма сериализации.
    // При десериализации JVM сравнивает serialVersionUID в файле с serialVersionUID в классе.
    // Если они не совпадают — выбрасывается InvalidClassException (файл несовместим).
    //
    // Зачем объявлять явно?
    // Если не объявить, JVM вычислит serialVersionUID автоматически на основе структуры класса:
    // имён полей и методов, их типов и модификаторов. Комментарии и Javadoc на UID не влияют.
    // Но даже добавление нового метода или поля изменит автоматический UID — и старые сохранения
    // перестанут загружаться. Явное объявление защищает от этой проблемы.
    //
    // private — доступен только внутри класса (механизм сериализации находит его через рефлексию).
    // static — принадлежит классу, а не объекту (не сериализуется как поле объекта).
    // final — значение неизменяемо (версия фиксирована).
    // long — тип поля (64-битное целое), требуется спецификацией Serializable.
    // 1L — начальная версия. L — суффикс литерала long (без него компилятор считает число int).
    ...
    //
    // При бинарной загрузке восстанавливаем шанс крита из сохранённого значения.
    // Конструктор Archer(name) устанавливает critChance = 0.3 (30%),
    // но в процессе игры значение может измениться (баффы, уровни).
    // Без этого сеттера critChance после загрузки всегда сбрасывался бы на 0.3.
    //
    // Тип double (8 байт, IEEE 754) — дробное число с двойной точностью.
